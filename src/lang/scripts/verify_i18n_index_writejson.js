...
const html = fs.readFileSync('src/html/pages/index.html', 'utf8');
...
let i18nKeys = [];
try {
  const i18n = fs.readFileSync('src/assets/js/utils/i18n.js','utf8');
  const re = /\n\s*([a-z0-9_]+)\s*:\s*"/gi;
  let m;
...
fs.writeFileSync('src/lang/scripts/verify_result.json', JSON.stringify(out, null, 2), 'utf8');
console.log('Saved results to src/lang/scripts/verify_result.json');
